/**
 * openmcp — the OpenMCP catalog client and reference server, on this box.
 *
 * The package is `@logicsrc/openmcp`, published from logicsrc/openmcp and
 * running at openmcp.logicsrc.com. Nothing here reimplements it; this is the
 * part that has to exist so `openmcp` is a command on a server like every
 * other one in this repo rather than a thing you remember to `npx`.
 *
 * INSTALLED rather than run through npx, for the same reason as agenticjobs
 * and codeburn: a lookup is run many times a day and dlx hits the registry
 * for metadata before it hands over on every one of them.
 *
 * Into a PRIVATE PREFIX. The package's own executable is called `openmcp`,
 * which is also the name of this wrapper, so a global install puts a second
 * `openmcp` on PATH and which one wins depends on the order of two
 * directories. If theirs wins, nothing looks broken and this file never runs.
 * If ours wins and we followed PATH, we would exec ourselves until the
 * process table gave out. resolveRunner only ever accepts a copy that is NOT
 * this wrapper.
 *
 * THE PACKAGE MANAGES ITS OWN INSTALL. `openmcp update`, `uninstall` and
 * `where` all read a manifest.json that upstream's curl installer writes
 * (`curl -fsSL https://openmcp.logicsrc.com/install.sh | sh`), and a copy put
 * here by npm has no such file. Upstream answers "not installed by the
 * installer" and exits, which on a box where cli-tools plainly did install it
 * is the misleading kind of wrong. So those three words are answered here
 * when the copy running is ours, and handed straight through when it is not.
 * See ownsInstallWord.
 *
 *   OPENMCP_BIN    run this executable instead: a checkout, or their installer
 *   OPENMCP_SPEC   what gets installed, when you want a pinned version
 *
 * Deliberately NOT set here: OPENMCP_HOME. That is upstream's variable for
 * finding its own manifest, and pointing it at a prefix with no manifest in
 * it would trade a clear message for a confusing one.
 */
...
export const PACKAGE = '@logicsrc/openmcp';
export const EXECUTABLE = 'openmcp';

/**
 * The floor the package declares.
 *
 * Higher than this repo's own floor of 22.18, so it is checked rather than
 * assumed: the catalog keeps its records in node:sqlite, which is Node 24, and
 * the failure on older Node is a missing built-in module named nowhere useful.
 */
...
export function vendorRoot(env: NodeJS.ProcessEnv = process.env): string {
  return join(dataHome(env), 'cli-tools', 'vendor', 'openmcp');
}
...
/**
 * Which catalog client to run.
 *
 * The `ours` case is the whole reason this function exists. This wrapper is
 * installed on PATH under the name `openmcp`, the same name the package's own
 * bin uses, so "is openmcp on PATH" answers yes on every box where this
 * command is installed. Acting on that answer is a fork bomb.
 */
export function resolveRunner(deps: ResolveDeps = {}): Runner {
  ...
  // An explicit override wins outright: a checkout, or a copy put there by
  // upstream's own installer, is a deliberate act and not ours to second-guess.
  const override = env.OPENMCP_BIN;
 ...
 * one we installed: a copy put there by upstream's installer, or pointed at
 * with OPENMCP_BIN, keeps upstream's behaviour exactly.
 */
...
export function prepareVendorDir(root: string): void {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: 'cli-tools-vendor-openmcp',
        version: '0.0.0',
        private: true,
        description: 'Prefix owned by profullstack/cli-tools. Managed by the openmcp command.',
      },
      null,
      2,
    )}\n`,
...
/**
 * Install (or refresh) the catalog client in the private prefix.
 *
 ...
 * thing for a copy installed that way and no business of ours. A catalog's
 * own database (`openmcp serve --db`) lives wherever it was pointed, never in
 * this prefix, so removing the program removes no records.
 */
...
export function installFailureMessage(root: string): string {
  return [
    `openmcp: could not install ${PACKAGE}.`,
    `  cd ${root} && npm install ${PACKAGE}@latest                 # by hand, to see the error`,
    '  curl -fsSL https://openmcp.logicsrc.com/install.sh | sh     # or upstream\'s installer, which brings its own Node',
    '  OPENMCP_BIN=/path/to/openmcp openmcp                         # or point at a copy you have',
  ].join('\n');
}

/** What to print when the Node running this is older than the package accepts. */
export function nodeFloorMessage(version: string): string {
  return [
    `openmcp: needs Node ${MIN_NODE} or newer, and this is ${version}.`,
    '  mise use -g node@lts       # then re-run',
    '  curl -fsSL https://openmcp.logicsrc.com/install.sh | sh   # or the installer, which fetches its own Node 24',
